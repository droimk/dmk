...
####droi huangjun add###########################################################
    __do_clone $(echo `echo $product | sed "s/full_//"`)
    if [ $? -ne 0 ];then
    	echo -e "\n__do_clone return `cat clperr.log`"
    	return 1
    fi
####end
...
################################################################################
#huangjun @ 2015.10.15~10.28
function mdroi()
{
	ONE_SHOT_MAKEFILE=droimk DROI_FC=$1 make 
}
...
function createproject()
{
	...
	old_path=`pwd`
	[ -z $1 ] && echo "Select a baseproduct:"
	sleep 1
	lunch $(echo $1)
	ret=$?
	if [ $ret -eq 1 ];then
		echo "Sorry... lunch return $ret"
		return 1
	fi
	arch=$(get_build_var TARGET_ARCH)
	manufacturer=$(echo $2)
	product=$(echo $3)
	echo "Now create product starting..."
	if [ -z $manufacturer ] || [ -z $product ];then
		echo -n "Please input the manufacturer name: "
		read manufacturer
	...
	if [ -z $manufacturer ] || [ -z $product ];then
		echo "Are you kide me?manufacturer=[$manufacturer] product=[$product] Fail!"
		return 1
	else
		manufacturer=$(echo $manufacturer)
		product=$(echo $product)
	fi
	local droi_p=`echo $TARGET_PRODUCT | sed s/full_//`
	mkdir -p $_top/droi_src/project/${manufacturer}-${product}
	echo "droi_src/${manufacturer}-${product} Create done."
	
	if [ -L vendor/$com_name/libs/$droi_p ];then
		mtkbproject=`readlink vendor/$com_name/libs/$droi_p`
	else
		mtkbproject=$droi_p
	fi
	nproject="$product"
	
	if [ ! -e $_top/droi_src/config/project.db ];then
		mkdir -p $_top/droi_src/config/
		> $_top/droi_src/config/project.db
	fi
	ready=`cat $_top/droi_src/config/project.db  | grep "$nproject |"`
	if [ -z "$ready" ];then
		chmod 774 $_top/droi_src/config/project.db
		echo "$nproject | ${manufacturer}-${product} | $droi_p $mtkbproject $nproject $arch $com_name" >> $_top/droi_src/config/project.db
		chmod 444 $_top/droi_src/config/project.db
	else
		echo "$nproject have been created at `echo $ready | awk -F"|" '{print $2}'`"
		return 1
	fi
	__do_clone $nproject
	ret=$?
	if [ $ret -eq 0 ];then
		...
		echo "add_lunch_combo full_$nproject-eng"  >> droi_src/config/project.db.sh
		echo "add_lunch_combo full_$nproject-user"  >> droi_src/config/project.db.sh
		echo "add_lunch_combo full_$nproject-userdebug" >> droi_src/config/project.db.sh
	else
		echo -e "\nCreateproject fail at:__do_clone return `cat clperr.log`"
		__remove_project_config $nproject
		droi_src/build/clone_project.sh "1 2 $nproject 4 5 rm"
		return 1
	fi
	cd $old_path > /dev/null
	echo -e "Now init env to $nproject ..."
	. build/envsetup.sh 2>&1 > /dev/null
	lunch full_$nproject-eng
	echo "Create done."
}

function __remove_project_config()
{
	local _top=$(gettop)
	local project=$1
	local line=
	...
	local var=
	chmod 774 $_top/droi_src/config/project.db
	line=`cat $_top/droi_src/config/project.db | grep -n "$project |" | awk -F":" '{print $1}'`
	sed -i ${line}d $_top/droi_src/config/project.db
	chmod 444 $_top/droi_src/config/project.db
	line=(`cat droi_src/config/project.db.sh | grep -n "$project" |awk -F":" '{print $1}'`)
	for var in ${line[@]}
	do
		tmp="$tmp -e ${var}d"
	done
	if [ ${#line[@]} -gt 0 ];then
		sed -i $tmp $_top/droi_src/config/project.db.sh
	fi
}
...
function removeproject()
{
	...
	local old_path=`pwd`
	if [ -z "$rproj" ];then
		echo "Pls select the project removed :"
		sleep 0.5
		lunch
		rproj=`echo $TARGET_PRODUCT | sed s/full_//`
		vendor_dir=$DROI_PCBA_PRODUCT
	fi
	rproj=`echo $rproj | sed s/full_// | awk -F"-" '{print $1}'`
	ready=`cat $_top/droi_src/config/project.db | grep "$rproj |"`
	if [ ! -z "$ready" ];then
		rproj=$(echo `echo $ready | awk -F"|" '{print $1}'`)
		vendor_dir=`echo $ready | awk -F"|" '{print $2}'`
	else
		echo "Sorry,project[$rproj] can't be removed ..."
		return 1
	fi
	
	clone_arg=$(echo `echo $ready | awk -F"|" '{print $3}'`)
	cd $_top
	droi_src/build/clone_project.sh "$clone_arg rm"
	ret=$?
	if [ $ret -eq 0 ];then
		__remove_project_config $rproj
		echo -e "\nRemove project [$rproj] Done."
	else
		echo -e "\nRemove project [$rproj] error :return `cat clperr.log`"
	fi
...
function __do_clone()
{
	...
	if [ ! -f $_top/droi_src/config/project.db ];then
		echo "" > $_top/droi_src/config/project.db
		echo "" > $_top/droi_src/config/project.db.sh
	fi
	ready=`cat $_top/droi_src/config/project.db | grep "$project |"`
	if [ ! -z "$ready" ];then
		export DROI_PCBA_PRODUCT=$(echo `echo $ready | awk -F"|" '{print $2}'`)
		if [ ! -d "$_top/device/droi/$project" ];then
			clone_arg=$(echo `echo $ready | awk -F"|" '{print $3}'`)
			droi_src/build/clone_project.sh "$clone_arg"
			ret=$?
			msg="\n__do_clone done."
		fi
	else
		export DROI_PCBA_PRODUCT=
	fi
	echo -e $msg
	cd $old_path > /dev/null
...
{
	local listp=`print_lunch_menu |grep "full_" | awk -F"full_" '{print $2}' | awk -F"-" '{print $1}' | sort -u`
	echo "$listp"
}
...
function __get_tp_of_origin_path()
{
	local tps=`listproject`
	local origin_path=$1
	for tp in $tps;
	do
		if [  ! -z `echo $origin_path | grep $tp` ];then
			echo $tp
			return 0
		fi
	done
...
function __gen_droi_mk()
{
	#$1 d2p origin #$2 d2p target
	local target=$1
	local ori=$2
	local _top=$(gettop)
	local target_droi_mk=
	local target_droi_dir=$_top/$ori/$target
	local tp=

	target_is_file=
	if [ -f "$target_droi_dir" ];then
		target_droi_dir=`dirname $target_droi_dir`
		target_is_file=1
	fi
	target_droi_mk=$target_droi_dir/droi.mk

	echo -n "Gen droi.mk... "
	tp=`__get_tp_of_origin_path $target`
	target=`echo $target | sed s#$tp#'$(DROI_PRODUCT)'#g`
	if [ "$droi_config_file" = "1" ];then
		local rule="`basename $target`:$target"
		if [ ! -f $_top/$ori/droi.mk ];then
			cp -f $_top/droi_src/build/droi_config.mk $_top/$target_dir/droi.mk
		fi
		if [ -z "`cat $_top/$ori/droi.mk | grep $rule`" ];then
			echo "DROI_NEED_REPLACE_MK += $rule" >> $_top/$ori/droi.mk
		fi
		droi_old_config_file=1
	elif [ "$target_is_file" = "1" ];then
		local filename=`basename $target`
		local droi_module=`echo $filename | sed "s#\\\\.#_#g"`
		if [ ! -f "$target_droi_mk" ];then
			echo -n "droi.mk NOT exsit.create "
			cp -f $_top/droi_src/build/droi_tmp.mk $target_droi_mk
			sed -i s#"DROI_MODULE :="#"DROI_MODULE := $droi_module"#g $target_droi_mk
			sed -i s#"DROI_SRC_FILE :="#"DROI_SRC_FILE := $filename"#g $target_droi_mk
			sed -i s#"DROI_SRC_TARGET_PATH :="#"DROI_SRC_TARGET_PATH := $target"#g $target_droi_mk
		else
			echo -n "droi.mk exsit.change "
			if [ -z "`cat $target_droi_mk | grep $droi_module`" ];then
				echo "" >> $target_droi_mk
				echo 'include $(DROI_CLEAR_VARS)' >> $target_droi_mk
				echo "DROI_MODULE := $droi_module"  >> $target_droi_mk
				echo "DROI_SRC_FILE := $filename" >> $target_droi_mk

				echo "DROI_SRC_TARGET_PATH := $target" >> $target_droi_mk
				echo 'include $(DROI_PREBUILD)' >> $target_droi_mk
			else
				echo "DROI_MODULE := $droi_module has been added!!ignore... "
			fi
		fi
	else
		cp -f $_top/droi_src/build/droi_tmp.mk $target_droi_mk
...
function d2p()
{
	...
	local recursive=""
	local _top=$(gettop)
	local target_dir=droi_src/project/$DROI_PCBA_PRODUCT
	local temp_path=
	local parents="--parents"
	droi_config_file=
	
	if [ -z "$TARGET_PRODUCT" ];then
		echo "Pls run lunch before this commond"
		return 1
	fi
	
	if [ -z `echo ${args[@]} | grep "\-undo"` ];then
		droi_old_config_file=0
	fi
	...
	do
		case $arg in
			...
			;;
			"-h")
				echo "d2p will cp source dir/file to project dir"
				echo "d2p [option] [source dir]"
				echo "option:"
				echo "  -h,show this help text"
				echo "  -r,copy directories recursively"
				echo "  -undo,roll back last copy-action,do rm"
				echo "***note***"
				echo "  -p,this option just for makefile xxx.mk,them will be copyed firstly."
				echo "     Files locate at $target_dir/config_mk"
				return 0
			;;
			"-p")
			#if XX.mk only ,this cmd can remove
				droi_config_file=1
				target_dir="$target_dir"/config_mk
				if [ ! -d $_top/$target_dir ];then
					mkdir $_top/$target_dir
				fi
				parents=
			;;
			"-undo")
				if [ ! -z "$droi_last_origin_relative_path" ];then
					local docf=$droi_last_origin_relative_path
					local tp=
					if [ "$droi_old_config_file" = "1" ];then
						target_dir="$target_dir"/config_mk
						echo "rm -f $_top/$target_dir/`basename $docf`"
						rm -f $_top/$target_dir/`basename $docf`
						tp=`__get_tp_of_origin_path $docf`
						docf=`echo $docf | sed s#$tp#'$\(DROI_PRODUCT\)'#g`
						local line=(`cat $_top/$target_dir/droi.mk | grep -n $docf | awk -F: '{print $1}'`)
						while [ ${#line[@]} -ne 0 ]
						do
							echo "sed -i ${line[0]}d $target_dir/droi.mk"
							sed -i ${line[0]}d $_top/$target_dir/droi.mk 
							line=(`cat $_top/$target_dir/droi.mk | grep -n $docf | awk -F: '{print $1}'`)
						done
						droi_last_origin_relative_path=
						echo "Undo Done."
					elif [ "$target_is_file" = "1" ];then
						local sl=
						local el=
						echo "rm -rf $_top/$target_dir/$docf"
						rm -rf $_top/$target_dir/$docf
						tp=`__get_tp_of_origin_path $docf`
						local docf_dir=`dirname $droi_last_origin_relative_path`
						docf=`echo $docf | sed s#$tp#'$\(DROI_PRODUCT\)'#g`
						local line=(`cat $_top/$target_dir/$docf_dir/droi.mk | grep -n $docf | awk -F: '{print $1}'`)

						((sl = line - 3))
						((el = line + 1))
						if [ ! -z $line ];then
							sed -i $sl,${el}d $_top/$target_dir/$docf_dir/droi.mk
						fi
						droi_last_origin_relative_path=
						echo "Undo Done."
					elif [ -e "$_top/$target_dir/$docf" ];then
						echo "rm -rf $_top/$target_dir/$docf"
						rm -rf $_top/$target_dir/$docf
						droi_last_origin_relative_path=
						echo "Undo Done."
					else
						echo "not find : $_top/$target_dir/$docf"
					fi
				fi
				return 0
			;;
			*)
				#echo "Unkown arg:$arg"
				temp_path=$arg
				local _name=`basename $temp_path`
				if [ ! -z "`echo $_name | grep ".mk"`" ];then
					echo "Droi: As a mk"
					droi_config_file=1
					target_dir="$target_dir"/config_mk
					if [ ! -d $_top/$target_dir ];then
	...
	if [ "$temp_path" = "." ];then
		temp_path=
	elif [ ! -z `echo "$temp_path" | grep  "./"` ];then
		temp_path=`echo "$temp_path" | sed "s#\\\\./##"`
	fi
	relative_path=`pwd`
	if [ $relative_path = $_top ];then
		relative_path=$temp_path
	else
		relative_path=`echo $relative_path | sed s#$_top/##`/$temp_path
	fi
	
	if [ ! -z `echo $relative_path |grep "droi_src"` ];then
		echo "error: droi_src is distance dir!"
		return 1
	fi
	if [ ! -e $_top/$relative_path ];then
		echo "Can't find the path/file [$relative_path]"
		return 1
	fi

	if [ "`echo $relative_path | awk -F/ '{print $1}'`" = "out" ];then
		echo "Forbiden dir:out"
		return 1
	fi
	droi_last_origin_relative_path=$relative_path
	cd $_top > /dev/null
	echo -e "cp \033[31m-uf $recursive $parents\033[0m \033[32m$relative_path\033[0m \033[30m$target_dir\033[0m"
	local ret=`cp -uf $recursive $parents $relative_path $target_dir 2>&1`
	if [ ! -z "$ret" ];then
		echo "[d2p]error:$ret May add -r for recursively"
		cd - > /dev/null
		return 1
	fi
	cd - > /dev/null
	__gen_droi_mk "$relative_path" "$target_dir"
	return 0
}
...
for f in `test -d device/droi && find -L device/droi -maxdepth 4 -name 'vendorsetup.sh' 2> /dev/null` \
         `test -d vendor/droi && find -L vendor/droi -maxdepth 4 -name 'vendorsetup.sh' 2> /dev/null` \
         droi_src/config/project.db.sh
do
    echo "including $f"
    . $f
done
